...
def get_dataset(config, epsilon=50, num_samples=2):
    ...
    quality_path = os.path.join(".", "data", "geolife", "quality")
    ...
    filtered_locs.as_locations.to_csv(os.path.join(".", "data", "geolife", f"locations_geolife.csv"))
    ...
    sp_time.to_csv(f"./data/geolife/sp_time_temp_geolife.csv", index=False)
...
def _filter_sp_history(sp):
    ...
    data_path = f"./data/geolife/valid_ids_geolife.pk"
    with open(data_path, "wb") as handle:
        pickle.dump(final_valid_id, handle, protocol=pickle.HIGHEST_PROTOCOL)
    filtered_sp.to_csv(f"./data/geolife/dataSet_geolife.csv", index=False)
