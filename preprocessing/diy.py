...
def get_dataset(config, epsilon=50, num_samples=2):
    ...
    raw_df = pd.read_csv(os.path.join(config["raw_diy"], "raw_diy_mobility_dataset.csv"))
    ...
    gdf = gdf.rename(columns={'tracked_at': 'tracked_at_str', 'geometry': 'geom'})
    
    # Parse timestamps and set timezone to Asia/Jakarta
    print("Parsing timestamps...")
    gdf['tracked_at'] = pd.to_datetime(gdf['tracked_at_str'])
    gdf['tracked_at'] = gdf['tracked_at'].dt.tz_convert('Asia/Jakarta')
    ...
    pfs, sp = pfs.as_positionfixes.generate_staypoints(
        gap_threshold=24 * 60, include_last=True, print_progress=True, 
        dist_threshold=100, time_threshold=30, n_jobs=-1
    )
    ...
    quality_path = os.path.join(".", "data", "diy", "quality")
    ...
    if Path(quality_file).is_file():
        ...
        quality_filter = {"day_filter": 60, "window_size": 10, "min_thres": 0.6, "mean_thres": 0.7}
    ...
    filtered_locs.as_locations.to_csv(os.path.join(".", "data", "diy", f"locations_diy.csv"))
    ...
    sp_time.to_csv(f"./data/diy/sp_time_temp_diy.csv", index=False)
...
def _filter_sp_history(sp):
    ...
    data_path = f"./data/diy/valid_ids_diy.pk"
    with open(data_path, "wb") as handle:
        pickle.dump(final_valid_id, handle, protocol=pickle.HIGHEST_PROTOCOL)
    filtered_sp.to_csv(f"./data/diy/dataSet_diy.csv", index=False)
...
if __name__ == "__main__":
    ...
    parser.add_argument("epsilon", type=int, nargs="?", help="epsilon for dbscan to detect locations", default=50)
