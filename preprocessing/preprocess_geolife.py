"""
Geolife dataset preprocessing script with YAML configuration support.

This script processes the raw Geolife dataset into the format required for 
...
def get_dataset(config):
    ...
    print(f"\nReading Geolife data from: {raw_data_path}")
    pfs, _ = read_geolife(raw_data_path, print_progress=sp_params['print_progress'])
    ...
    locations_file = os.path.join(output_dir, f"locations_geolife.csv")
    ...
    intermediate_file = os.path.join(output_dir, "sp_time_temp_geolife.csv")
...
def _filter_sp_history(sp, config):
    ...
    dataset_path = os.path.join(output_dir, "dataSet_geolife.csv")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Preprocess Geolife dataset")
    parser.add_argument(
        "--config",
        type=str,
